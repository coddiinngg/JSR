...
import { ChevronLeft, MoreVertical, Flame, Camera, CheckCircle2, X, Edit3, Trash2, Pause } from "lucide-react";
import { useNavigate, useParams, Link } from "react-router-dom";
import { cn } from "../lib/utils";
import { useApp } from "../contexts/AppContext";
import { inferVerifyTypeForGoalCategory } from "../lib/verifyTypes";
...
export function GoalDetail() {
  ...
  const { goals, beginVerification } = useApp();
  const goal = GOALS[id] ?? GOALS["1"];
  const currentGoal = goals.find(item => item.id === id) ?? null;
  const verifyType = inferVerifyTypeForGoalCategory(currentGoal?.category ?? "exercise");
  const color = currentGoal?.color ?? goal.color;
  const colorRgb = currentGoal?.colorRgb ?? goal.colorRgb;
  const title = currentGoal?.title ?? goal.title;
  const sub = goal.sub;
  const streak = currentGoal?.streak ?? goal.streak;
  const rate = currentGoal?.progress ?? goal.rate;
  const total = goal.total;
  const completionRate = goal.completionRate;
  ...
  const startGoalVerification = () => {
    beginVerification({ goalId: id, verifyType });
  };
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 pb-28">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            ...
            <Link
              to={`/verify/guide/${verifyType}`}
              onClick={startGoalVerification}
