-- ============================================================
-- 강퇴 타이머 변경: 48h 미인증 → 경고 + 6h 유예 → 퇴장
-- (기존: 48h 경고, exit_deadline = 마지막인증 + 72h)
-- (신규: 48h 경고, exit_deadline = NOW() + 6h)
-- ============================================================
...
BEGIN
  ...
  -- 48시간 미인증 시 경고, 6시간 유예 부여
  WITH changed AS (
    UPDATE group_members
    SET member_status = 'EXIT_ELIGIBLE',
        exit_deadline = NOW() + INTERVAL '6 hours'
  ...
  SELECT
    ...
    'member_warning',
    '퇴장경고',
    '48시간 이상 미인증으로 퇴장경고 상태가 됐어요. 6시간 이내에 인증하지 않으면 ' || g.name || '에서 퇴장돼요.',
    '⚠️',
    c.group_id::TEXT
  FROM changed c
  ...
  -- exit_deadline(= 경고 시점 + 6h) 경과 시 퇴장
  WITH changed AS (
    UPDATE group_members
  ...
  SELECT
    ...
    'member_removed',
    '그룹에서 퇴장됐어요',
    g.name || '에서 48시간 미인증으로 자동 퇴장 처리됐어요.',
    '🚪',
    c.group_id::TEXT
  FROM changed c
  JOIN groups g ON g.id = c.group_id;
